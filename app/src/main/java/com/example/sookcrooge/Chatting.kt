...
class Chatting : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        userUID=loginInformation.currentLoginUser?.uid.toString()
        userNickname=loginInformation.currentLoginUser?.nickname.toString()
        ...
        val chattingUsersQuery = db.collection("rooms").document(documentID).collection("chatUsers")
        chattingUsersQuery.addSnapshotListener{snapshot, e ->
                                                ...
                                                else
                                                {
                                                    ...
                                                    (binding.itemRecycler.adapter as OthersAccountAdapter).setItemClickListener(object: OthersAccountAdapter.OnItemClickListener{
                                                        override fun onSmileClick(binding: AccountListBinding, data: accountItem) {
                                                            ...
                                                            db.collection("users").document(othersUserUID).collection("accountBook").document(data.documentID).update("smile", (data.smile+1))
                                                        }
                                                        override fun onAngryClick(binding: AccountListBinding, data: accountItem)
                                                        {
                                                            ...
                                                            db.collection("users").document(othersUserUID).collection("accountBook").document(data.documentID).update("angry", (data.angry+1))
                                                        }
                                                    })
                                                }
                                                runOnUiThread{
            ...
            /*
            val chattingUsers = snapshot!!.documents.data?.get("userName").toString()
